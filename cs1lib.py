...
#         self.image_painter.setRenderHint(QPainter.Antialiasing, True)
  ...
  #              self.image_painter.drawImage(x, self.height - y, image)
    ...
    #For drawing from selected areas of spritesheets 
    #coordinates for spritesheet: xl(x left), yt(y top)
    def advanced_draw_image(self, image, xl, yt, x, y, width, height, scale):
        
        with self.lock_image:
            
            slice_rect = QRectF(x, y, width, height)
            
            if self.flipped_y:
                self.image_painter.save()
                #self.image_painter.translate(0, self.height)
                self.image_painter.scale(scale, scale*-1)
                
                p = QPointF(xl, ((self.height * scale) - yt - (image.height() * scale)))
                
                self.image_painter.drawImage(p, image, slice_rect)
                
                self.image_painter.restore()
             
            else:   
                
                self.image_painter.scale(scale, scale)
                p = QPointF(xl, yt)
...
#For drawing from selected areas of spritesheets 
def advanced_draw_image(image, xl, yt, x, y, width, height, scale, cx = 0, cy = 0, theta = 0):
    push_state()
    ...
    #The only difference from draw_image, calling the version that includes cropping from an area of an image
    canvas.advanced_draw_image(image, xl, yt, x, y, width, height, scale)
...
#The following is code by Justin Ruiz that was not used:
#def draw_area(self, xl, xr, yt, yb):
#        width = xr - xl
#        height = yb - yt
#        rgba = (0, 0, 0, 0)
#
...
#                rgba = self.get_pixel(x, y)
#                
#                self.set_pixel((xl + x), (yt + y), rgba[0], rgba[1], rgba[2], rgba[3])
